...
const OracleDuplicateColumnError = '42701';
const OracleMissingColumnError = '42703';
const OracleUniqueIndexViolationError = '23505';
const logger = require('../../../logger');

const debug = function (...args: any) {
  args = ['ORA: ' + arguments[0]].concat(args.slice(1, args.length));
...
const getOracleConnection = async () => {
  ...
  console.log('MARK: got connection');
  return connection;
};
...
export class OracleStorageAdapter implements StorageAdapter {
  ...
  constructor({ uri, collectionPrefix = '', databaseOptions = {} }: any) {
    ...
    console.log('MARK: in OracleStorageAdapter.constructor()');
    const { client, pgp } = createClient(uri, databaseOptions);
    console.log('MARK: client version = ' + client.oracleClientVersion);
    this._client = client;
    this._onchange = () => {};
    this._pgp = pgp;
    this._uuid = uuidv4();
  ...
  async _ensureSchemaCollectionExists(conn: any) {
    console.log(
      'MARK: in _ensureSchemaCollectionExists and client version = ' +
        this._client.oracleClientVersion
    );
      ...
      begin
        stmt := q'{
          CREATE TABLE "_SCHEMA" (
            ...
            "schema" clob, 
            "isParseClass" number(1,0), 
            constraint parse_schema_pk PRIMARY KEY ("className"), 
            constraint parse_ensure_json check ("schema" is json) 
          )
        }';
